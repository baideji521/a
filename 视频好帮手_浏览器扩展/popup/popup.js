...
const APP_URL = "https://github.com/tonhowtf/视频好帮手/releases/latest";
...
function renderMediaDetected(container) {
  ...
  const directVideo = nonHls.filter(m => m.contentLength > 500 * 1024 && m.mediaType === "video");
  const totalVideos = groups.length + directVideo.length;

  const domain = getDomainFromUrl(currentData.tabUrl || best.url);
  const title = pageTitle ? truncate(pageTitle, 40) : domain;

  if (totalVideos > 1) {
    // 多个视频 → 主按钮批量发送所有嗅探到的视频
    const batchItems = [
      ...groups.map(g => ({ type: "hls", master: g.master })),
      ...directVideo.map(m => ({ type: "direct", media: m })),
    ];
    const countText = tr("popup_videos_found", totalVideos);
    const label = tr("popup_send_all", totalVideos);
    appendPrimaryButton(container, label, title + " \u00b7 " + countText, (btn) => {
      handleBatchDownload(btn, batchItems);
    });
  } else {
    // 单个视频 → 正常发送
    const countText = totalVideos > 0 ? tr("popup_video_found_one") : "";
    const label = best.mediaType === "audio" ? tr("popup_send_audio") : tr("popup_send_default");
    const meta = countText ? title + " \u00b7 " + countText : title;
    appendPrimaryButton(container, label, meta, (btn) => {
...
function sendToApp(url, platform, mediaEntry) {
  return new Promise((resolve) => {
    const msg = { type: "sendTo视频好帮手", url, platform };
...
async function sendBatch(groups) {
  ...
  for (const item of groups) {
    // 支持 HLS 组（有 .master）和直接视频（有 .media）
    const entry = item.master || item.media;
    if (!entry) continue;

    const msg = { type: "sendTo视频好帮手", url: entry.url, platform: "generic" };
    if (currentData?.tabUrl) msg.referer = currentData.tabUrl;
    if (entry.mediaType) msg.mediaType = entry.mediaType;
    if (entry.contentType) msg.contentType = entry.contentType;
    msg.openApp = isOpenAppEnabled();
